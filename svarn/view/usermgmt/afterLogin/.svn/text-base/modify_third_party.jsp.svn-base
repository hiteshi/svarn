<div class="polo">

	<table>
		<tr>
			<td><div
					style="height: 100px; width: 416px; border-bottom: 1px solid red;">
					list all own account : transfer from account</div>
			</td>
			<td><div
					style="height: 100px; width: 416px; border-bottom: 1px solid blue;">list
					all own account : transfer to account</div>
			</td>
		</tr>
	</table>

	<form action="">
		<table height="300" width="850">
			<tr>MODIFY THIRD PARTY
			</tr>
			<tr>
				<td>Account Number</td>
				<td>:</td>
				<td><select id="auto_accno">
						<option value="op1">Hiiiiiiiiiiiiiiiii</option>
						<option value="op2">abc</option>
						<option value="op3">cdef</option>
						<option value="op4">hello</option>
				</select></td>
			</tr>
			<tr>
				<td>Nick Name</td>
				<td>:</td>
				<td><input id="nickname" type="text" maxlength="15" /></td>
			</tr>
			<tr>
				<td>Transfer Limit</td>
				<td>:</td>
				<td><input type="text" id="limit" maxlength="6" />
				</td>
			</tr>
			<tr>
				<td align="right" colspan="3"><input id="countinue"
					type="submit" value="Add" /></td>
			</tr>
		</table>
	</form>
</div>
<div style="height: 50px; width: 850px; border-top: 1px solid red">list
	all own account : transfer to account</div>
</div>