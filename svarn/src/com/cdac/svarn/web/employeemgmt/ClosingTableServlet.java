...
public class ClosingTableServlet extends HttpServlet {
	...
	throws IOException
	{
		...
		try{
			...
			ArrayList<RegisterClosing> users = emb.getClosings();
			...
			for(;i<users.size();i++)
			{
				String link="<td><a href=\"#\" onclick=\"JavaScript:xmlhttpPost("+i+","+users.get(i).getClosingId()+");\">"+users.get(i).getClosingId()+"</a>";
				writer.println(""+link);
				writer.println("</td>");
				writer.println("<td>"+users.get(i).getAccountNo()+"</td>");
				writer.println("</tr>");
			}
