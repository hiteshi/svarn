...
public class CompanyTableServlet extends HttpServlet {
	...
	throws IOException
	{
		...
		try{
			...
			ArrayList<RegisterCompany> users = emb.getCompanies();
			...
			for(;i<users.size();i++)
			{
				String link="<td><a href=\"#\" onclick=\"JavaScript:xmlhttpPost("+i+","+users.get(i).getRegistrationId()+");\">"+users.get(i).getRegistrationId()+"</a>";
				writer.println(""+link);
				writer.println("</td>");
				writer.println("<td>"+users.get(i).getCompanyName()+"</td>");
				writer.println("</tr>");
			}
